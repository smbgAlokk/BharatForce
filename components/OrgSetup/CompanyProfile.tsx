...
export const CompanyProfile: React.FC = () => {
  const { currentTenant, updateCompany } = useApp();
  const [formData, setFormData] = useState(currentTenant);
  ...
  if (!currentTenant) return <div>No company selected</div>;
          ...
          <div className="border-t border-slate-100 pt-6">
             ...
             <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
               <div className="sm:col-span-6">
                  ...
                  <input type="text" value={formData?.addressLine1 || ''} onChange={e => handleChange('addressLine1', e.target.value)}
                    className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                </div>
                <div className="sm:col-span-2">
