...
export type EmploymentStatus = 'Active' | 'Probation' | 'Notice Period' | 'Separated' | 'Onboarding';
...
export type DocumentType = 'Aadhaar' | 'PAN' | 'Photo' | 'Appointment Letter' | 'Offer Letter' | 'ID Card Copy' | 'Previous Experience Letter' | 'Educational Certificate' | 'Address Proof' | 'Bank Proof' | 'Other';
...
export interface ProfileChangeRequest extends AuditFields {
  ...
  employeeName: string;
  employeeCode: string;
  category: 'Personal Details' | 'Contact Details' | 'Bank Details' | 'Family Details' | 'IDs' | 'Address';
...
export type AttendanceStatus = 'Present' | 'Absent' | 'Half Day' | 'Leave' | 'Holiday' | 'Weekly Off' | 'On Duty' | 'Work From Home';
...
export interface AttendancePolicy extends AuditFields {
  ...
  exceptionRules?: {
    lateComing: { enabled: boolean; graceMinutes: number; thresholdMinutes?: number; maxLateMarksPerMonth?: number };
    earlyExit: { enabled: boolean; graceMinutes: number; thresholdMinutes?: number };
    shortHours: { enabled: boolean; minFullDayMinutes: number; minHalfDayMinutes: number };
  };
  
  isActive: boolean;
}

export type MappingLevel = 'DefaultCompany' | 'Department' | 'Designation' | 'Employee' | 'Grade';
...
export type RegularisationType = 'Missed Punch' | 'Wrong Time Correction' | 'Mark Present' | 'Work From Home' | 'On Duty' | 'Shift Change' | 'Half Day';
export type RequestStatus = 'Draft' | 'Pending Manager Approval' | 'Pending HR Approval' | 'Approved' | 'Rejected' | 'Cancelled';
...
export type PayrollDayType = 'Paid_Full' | 'Paid_Half' | 'LOP_Full' | 'LOP_Half' | 'Paid_WeeklyOff' | 'Paid_Holiday' | 'Paid_OnDuty' | 'Paid_WFH' | 'Custom';
...
export type AccrualFrequency = 'Monthly' | 'Quarterly' | 'Half-Yearly' | 'Yearly' | 'On DOJ Anniversary' | 'Lump Sum';
export type AccrualMode = 'Start of Period' | 'End of Period' | 'Pro-rated by DOJ';
...
export interface LeaveBalanceChangeLog extends AuditFields {
  ...
  leaveTypeId: string;
  changeDate: string;
  changeSource: 'Accrual' | 'Manual Adjustment' | 'Leave Application' | 'Encashment' | 'Carry Forward';
  referenceId?: string;
  oldBalance: number;
  newBalance: number;
  changeAmount: number;
...
export type EncashmentStatus = 'Submitted' | 'Approved' | 'Rejected' | 'Processed';
...
export type CalculationType = 'Flat Amount' | 'Percentage of Basic' | 'Percentage of CTC' | 'Percentage of Remaining CTC';
...
export type AllocationType = 'Percentage of CTC' | 'Percentage of Basic' | 'Percentage of Remaining CTC' | 'Flat Amount (Monthly)' | 'Flat Amount (Yearly)';
...
export type ClaimStatus = 'Draft' | 'Submitted' | 'Manager Approved' | 'HR Approved' | 'Rejected' | 'Paid';
...
export type AdvanceStatus = 'Draft' | 'Submitted' | 'Manager Approved' | 'HR Approved' | 'Rejected' | 'Closed';
...
export type ResignationStatus = 'Draft' | 'Submitted' | 'Manager Approved' | 'Manager Rejected' | 'HR Approved' | 'HR Rejected';
...
export type CandidateStage = 'New / Applied' | 'Screening' | 'Interview' | 'Offer' | 'Hired' | 'Rejected' | 'Withdrawn';

export const CANDIDATE_STAGES: CandidateStage[] = ['New / Applied', 'Screening', 'Interview', 'Offer', 'Hired', 'Rejected', 'Withdrawn'];
...
export type ChecklistGroup = 'HR Checklist' | 'IT Checklist' | 'Admin Checklist' | 'Manager Checklist';
...
export type ScoringMethod = 'Rating_1_5' | 'Rating_1_10' | 'Percentage' | 'Binary_Yes_No';
...
export type PerformanceFrequency = 'Monthly' | 'Quarterly' | 'Half-Yearly' | 'Yearly' | 'On-demand';
...
export interface AppraisalForm extends AuditFields {
  ...
  selfAppraisal?: { summary?: string; achievements?: string; submittedOn?: string; submittedBy?: string };
  managerAppraisal?: { summary?: string; recommendation?: string; submittedOn?: string; submittedBy?: string };
  hrAppraisal?: { summary?: string; submittedOn?: string; submittedBy?: string };
}
...
export interface FeedbackQuestion extends AuditFields {
  ...
  companyId: string;
  text: string;
  category: 'Behavioural' | 'Leadership' | 'Core Values' | 'KPI-related' | 'Other';
...
export type RaterStatus = 'Pending' | 'Invited' | 'Started' | 'Submitted' | 'Declined';
