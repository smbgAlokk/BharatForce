...
export const EducationTab: React.FC<Props> = ({
  ...
  const saveEducation = (e: React.FormEvent) => {
    ...
    const updatedList = currentEdu.id
      ? (employee.education || []).map((x) =>
          x.id === currentEdu.id ? newEdu : x
        )
  ...
  const saveExperience = (e: React.FormEvent) => {
    ...
    const updatedList = currentExp.id
      ? (employee.experience || []).map((x) =>
          x.id === currentExp.id ? newExp : x
        )
  ...
  const deleteExperience = (id: string) => {
    if (window.confirm("Delete this experience record?")) {
      const updatedList = (employee.experience || []).filter(
        (x) => x.id !== id
      );
      onSave({ experience: updatedList });
    }
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">
                {currentEdu.id ? "Edit" : "Add"} Education
              </h3>
              <button
            ...
            <form onSubmit={saveEducation} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={currentEdu.yearOfPassing || ""}
                    onChange={(e) =>
                      setCurrentEdu({
                        ...currentEdu,
                        yearOfPassing: parseInt(e.target.value),
                      })
                    }
                    className="mt-1 block w-full border border-slate-300 rounded-md py-2 px-3 focus:ring-indigo-500 sm:text-sm"
                  />
      ...
      {expModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900 bg-opacity-50 px-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">
                {currentExp.id ? "Edit" : "Add"} Experience
              </h3>
              <button
            ...
            <form onSubmit={saveExperience} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCurrentExp({ ...currentExp, jobTitle: e.target.value })
                  }
                  className="mt-1 block w-full border border-slate-300 rounded-md py-2 px-3 focus:ring-indigo-500 sm:text-sm"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={currentExp.lastCtc || ""}
                  onChange={(e) =>
                    setCurrentExp({ ...currentExp, lastCtc: e.target.value })
                  }
                  className="mt-1 block w-full border border-slate-300 rounded-md py-2 px-3 focus:ring-indigo-500 sm:text-sm"
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={currentExp.reasonForLeaving || ""}
                  onChange={(e) =>
                    setCurrentExp({
                      ...currentExp,
