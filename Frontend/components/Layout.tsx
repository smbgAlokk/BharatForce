...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  // ✅ FIX 1: Removed 'switchUserRole', Added 'logout'
  const {
    currentUser,
    currentTenant,
    companies,
  ...
  // Fallback check in case currentUser is null (shouldn't happen due to ProtectedRoute, but good for safety)
  ...
  useEffect(() => {
    ...
    employees.forEach((emp) => {
      if (
        emp.companyId === currentTenant.id &&
    ...
    jobRequisitions.forEach((job) => {
      if (
        job.companyId === currentTenant.id &&
    ...
    candidates.forEach((cand) => {
      if (
        cand.companyId === currentTenant.id &&
  ...
  // Safe check for User (prevents crash if null)
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      >
          ...
          {currentUser.role === UserRole.SUPER_ADMIN ? (
            ...
            <div className="flex items-center space-x-3">
              <div
                ...
                style={{
                  backgroundColor: currentTenant?.primaryColor || "#3A5BA0",
                }}
              >
        ...
        {/* ✅ FIX 2: Secure User Profile & Logout */}
        <div className="p-4 border-t border-slate-800 bg-slate-950 relative">
          <div
            className="flex items-center space-x-3 cursor-pointer hover:bg-slate-900 p-2 rounded-lg transition-colors"
          ...
          {/* User Menu Dropdown (Replaced Switcher with Logout) */}
          {userMenuOpen && (
            <div className="absolute bottom-20 left-4 right-4 bg-white rounded-lg shadow-xl py-1 text-slate-900 text-sm z-50 border border-slate-200">
              ...
              <button
                ...
                className="w-full text-left px-4 py-2 hover:bg-indigo-50 text-slate-700 flex items-center"
              >
              ...
              <button
                onClick={() => {
                  ...
                  logout(); // ✅ Calls the secure logout function
                }}
                className="w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 border-t border-slate-100 flex items-center"
              >
                    ...
                    searchResults.map((result) => (
                      ...
                      >
                        ...
                        <div className="ml-3">
                          ...
                          <p className="text-xs text-slate-500 truncate w-56">
                            {result.subtitle}
                          </p>
